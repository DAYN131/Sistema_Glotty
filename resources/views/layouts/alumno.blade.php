...
<head>
    ...
    <title>@yield('title', 'Alumno - Glotty')</title>
...
<body class="bg-gray-50 min-h-screen flex flex-col font-sans antialiased text-text-primary">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        <!-- Top Header con colores suaves - Azul para alumno -->
        <header class="bg-gradient-to-r from-blue-600 to-blue-700 shadow-sm h-20 flex items-center px-8 text-white">
            <div class="flex items-center space-x-4">
                <div class="bg-white/10 p-3 rounded-xl backdrop-blur-sm">
                    <i class="fas fa-user-graduate text-white text-2xl"></i>
                </div>
                <div>
                    <h1 class="text-white font-bold text-2xl">@yield('header-title', 'Panel del Alumno')</h1>
                    <p class="text-blue-200 text-sm">Sistema Académico Glotty</p>
                </div>
            </div>
            <div class="ml-auto flex items-center space-x-6">
                <div class="text-right">
                    <p class="text-white font-semibold">{{ $nombre_completo ?? Auth::user()->nombre_completo }}</p>
                    <p class="text-blue-200 text-sm">Control: {{ $identificador ?? Auth::user()->numero_control }}</p>
                </div>
